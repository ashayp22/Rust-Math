...
impl SierpinskiCarpet {
    ...
    fn options_ui(&mut self, ui: &mut Ui) {
        ...
        if self.show_randomness {
            ui.add(Slider::new(&mut self.randomness_probability, 0.0..=1.0).text("Randomness Probability"));
        }
    ...
    fn paint_rect(&mut self, center: Pos2, size: Vec2, color: Color32) {
     ...
     fn sierpinski_carpet(&mut self, center_x: f32, center_y: f32, width: f32, height: f32, n: i64, level: i64) {
        ...
        if can_draw {
            ...
            let blue_colors = vec![Color32::from_rgb(144, 224, 239), Color32::from_rgb(72, 202, 228), Color32::from_rgb(90, 180, 216), Color32::from_rgb(0, 150, 199), Color32::from_rgb(0, 119, 182), Color32::from_rgb(2, 62, 138), Color32::from_rgb(3, 4, 94)];

            // draw in middle square
            if self.show_blue_shades {
                if level >= 0 && level <= 6 {
                    let color = blue_colors[level as usize];
                    self.paint_rect(pos2(center_x, center_y), vec2(width / 3.0, height / 3.0), color);
                } else {
                    self.paint_rect(pos2(center_x, center_y), vec2(width / 3.0, height / 3.0), Color32::WHITE);
                }
            } else {
                self.paint_rect(pos2(center_x, center_y), vec2(width / 3.0, height / 3.0), Color32::BLUE);
            } 
        }
            ...
            for row in -1..=1 {
                for col in -1..=1 {
                    if row != 0 || col != 0 {
                        ...
                        self.sierpinski_carpet(new_center_x, new_center_y, width / 3.0, height / 3.0, n - 1, level + 1);
                    }
                }
            }
        }
    ...
    fn paint(&mut self, painter: &Painter) {
        ...
        let need_to_recalculate = self.depth != self.last_depth || self.show_blue_shades != self.last_show_blue_shades || self.show_randomness != self.last_show_randomness || self.randomness_probability != self.last_randomness_probability;
        ...
        if need_to_recalculate {
            ...
            self.sierpinski_carpet(rect.width() / 2.0, rect.height() / 2.0, 500.0, 500.0, num_levels, 0);    
        }
